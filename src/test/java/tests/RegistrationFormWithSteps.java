...
public class RegistrationFormWithSteps {
    ...
    @Feature("–егистрационна€ форма студента") //название функциональности
    @Story("«аполнение формы и отправка данных студента") //краткое общее описание дл€ группировки тестов
    @DisplayName("ѕроверка соответстви€ введеных данных студента в форму и отображаемых значений в таблице")
    @Description(
            "Ётот тест провер€ет заполнение всех полей регистр. формы и корректное отображение введенных данных в таблице"
    ) //подробное описание
